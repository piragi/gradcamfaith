...
def preprocess_hyperkvasir_with_ssl4gie_transforms(
    ...
    save_split_metadata(train_df, val_df, test_df, config.file.data_dir)

    # Save transform information
    save_ssl4gie_transform_info(config.file.data_dir)
        ...
        try:
            ...
            if split_name == 'train' and save_multiple_augmentations:
                ...
                for aug_idx in range(num_augmentations):
                    new_filename = f"{video_file}_{finding}_aug{aug_idx}.jpg"
                ...
                # Save single version
                new_filename = f"{video_file}_{finding}.jpg"
                output_path = output_dir / new_filename
...
def run_hyperkvasir_ssl4gie_preprocessing(
    ...
    Saves processed images to train/val/test folders.
