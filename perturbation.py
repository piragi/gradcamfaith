...
def load_sd_model(
    model_path: str = "IrohXu/stable-diffusion-mimic-cxr-v0.1"
) -> StableDiffusionInpaintPipeline:
    sd_pipe = StableDiffusionInpaintPipeline.from_pretrained(
        model_path,
        torch_dtype=torch.float16
        if device == "cuda" else torch.float32).to(device)
    sd_pipe.scheduler = DDIMScheduler.from_config(sd_pipe.scheduler.config)
    return sd_pipe


def patch_similarity(original_patch: Image.Image,
                     perturbed_patch: Image.Image):
    ...
    # Calculate SSIM
    similarity = ssim(original_tensor,
                      perturbed_tensor,
                      data_range=1.0,
                      channel_axis=2)
    return similarity
...
def generate_patch_coordinates(image_width: int, image_height: int,
...
def create_patch_id(original_filename: str,
                    patch_id: int,
                    x: int,
                    y: int,
                    method: str = "sd",
                    strength: float = 0.2) -> str:
    if method == "sd":
        return f"{original_filename}_patch{patch_id}_x{x}_y{y}_s{strength}"
    else:
        return f"{original_filename}_patch{patch_id}_x{x}_y{y}_{method}"


def perturb_patch_sd(model_pipe: StableDiffusionInpaintPipeline,
                     image_identifier: str,
                     patch_info: Tuple[int, int, int],
                     strength: float = 0.2) -> Tuple[Image.Image, np.ndarray]:
    """
    ...
    """
    #TODO: change identifier handling
    patient_id, original_filename = image_identifier
    x, y, patch_size = patch_info

    original_512 = Image.open(
        f'./chexpert/{patient_id}/study1/{original_filename}.jpg').resize(
            (512, 512), Image.LANCZOS).convert('RGB')
...
def perturb_patch_mean(image_identifier: str, patch_info: Tuple[int, int, int],
    ...
    patch = Image.new("L", (patch_size, patch_size), mean_color)
